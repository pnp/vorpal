  ...
  if (args.length > 0 || catchExists) {
    if (options.use === 'minimist') {
      ...
      for (var i = 0; i < args.length; ++i) {
        ...
        if (args[i].indexOf(' ') > -1 &&
        // Waldek: added extra check to allow using JSON and XML values
        args[i].indexOf('"') === -1) {
          args[i] = '"' + args[i] + '"';
        }
      }
...
vorpal._exec = function (item) {
  ...
  if (match) {
    ...
    // Waldek: move 'Invalid command' forward so that it returns non-zero
    // exit code when command not found in non-immersive mode
    return callback(item, 'Invalid command.', undefined);
  }
};
...
vorpal._commandHelp = function (command) {
  ...
  for (var i = 0; i < this.commands.length; ++i) {
    var names = [].concat(this.commands[i]._name, this.commands[i]._aliases);
    allNames = allNames.concat(names);
    for (var k = 0; k < names.length; k++) {
      var parts = String(names[k]).split(' ');
  ...
  var commands = matches.filter(function (cmd) {
    ...
    return [cmd._name + (cmd._aliases.length > 0 ? '|' + cmd._aliases.join('|') : '') + (cmd.options.length ? ' [options]' : '') + ' ' + args, cmd.description() || ''];
  });
  ...
  var groups = _.uniq(matches.filter(function (cmd) {
    var names = [].concat(cmd._name, cmd._aliases);
    var hasMatch = false;
    for (var i = 0; i < names.length; i++) {
      if (String(names[i]).trim().split(' ').length > commandMatchLength) {
        hasMatch = true;
        break;
      }
    }
    return hasMatch;
  }).flatMap(function (cmd) {
    var names = [].concat(cmd._name, cmd._aliases);
    if (commandMatch) {
      for (var i = 0; i < names.length; i++) {
        if (String(names[i]).startsWith(command)) {
          return String(names[i]).split(' ').slice(0, commandMatchLength).join(' ');
        }
      }
    } else {
      return names.map(function (name) {
        return name.split(' ').slice(0, commandMatchLength).join(' ');
      });
    }
  }).map(function (cmd) {
